...
flags.DEFINE_integer(
    "lr_schedule_interval", 4000, "Number of steps between LR changes."
)
flags.DEFINE_float("lr_warmup", 0, "Learning rate warmup (epochs).")

# Model
flags.DEFINE_string(
    "architecture", "TransformerMDN", "Class name of model architecture."
)
...
def train(train_batches, valid_batches, output_dir=None, verbose=True):
    ...
    model = create_model(model_rng, input_shape, lm_kwargs, batch_size, verbose=verbose)
    ...
    for epoch in range(FLAGS.epochs):
        ...
        for step, batch in enumerate(tfds.as_numpy(train_batches)):
            ...
            optimizer, train_metrics = train_step(
                batch, optimizer, lr_scheduler(global_step)
            )
