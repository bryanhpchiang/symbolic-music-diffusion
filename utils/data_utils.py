...
def compute_dataset_cardinality(
    ds, ds_split="train", cache=False, cache_dir=None, config=""
):
    """Computes and optionally caches cardinality of tf.data.Dataset."""
    card_cache_path = os.path.join(
        cache_dir, f"cache/{ds_split}_{config}_cardinality.pkl"
    )
...
def compute_dataset_statistics(
    ...
    mean_cache_path = os.path.join(cache_dir, f"cache/{ds_split}_{config}_mean.pkl")
    stddev_cache_path = os.path.join(cache_dir, f"cache/{ds_split}_{config}_stddev.pkl")
    ...
    if os.path.exists(mean_cache_path) and os.path.exists(stddev_cache_path):
        ...
        ds_sum = ds.reduce(0.0, lambda x, y: x + y).numpy()
        ds_squared = ds.map(lambda x: x ** 2, num_parallel_calls=AUTOTUNE)
        ds_squared_sum = ds_squared.reduce(0.0, lambda x, y: x + y).numpy()
...
def get_tf_record_dataset(
    file_pattern=None, shape=(512,), batch_size=512, shuffle=True, tokens=False
):
    ...
    filenames = tf.data.Dataset.list_files(
        os.path.expanduser(file_pattern), shuffle=shuffle
    )
...
def infill_bars(embeddings, chunk_params, erased_chunk_indices):
    ...
    return jax.ops.index_update(
        embeddings, jax.ops.index[erased_chunk_indices], chunk_params
    )
