...
class ReplayBuffer(object):
    ...
    def add(self, samples):
        ...
        return self.replace(buffer_size=self.buffer_size, dims=self.dims, data=new_data)
...
def create_noise_schedule(sigma_begin=1, sigma_end=1e-2, L=10, schedule="geometric"):
...
def annealed_langevin_dynamics(
    ...
    collection_idx = jnp.linspace(1, len(sigmas) * T, collection_steps).astype(
        jnp.int32
    )
    ...
    def langevin_step(params, i):
        ...
        y = infill_samples + sigma * jax.random.normal(
            key=infill_rng, shape=infill_samples.shape
        )

        grad = model(state, sigma)
        noise = jnp.sqrt(2 * alpha) * jax.random.normal(key=step_rng, shape=state.shape)
        ...
        step_norm = jnp.sqrt(jnp.sum(jnp.square(alpha * grad), axis=1) + 1e-10).mean()
    ...
    def sample_with_sigma(params, sigma_i):
        ...
        next_ld_state, metrics = jax.lax.scan(langevin_step, ld_params, jnp.arange(T))
    ...
    ld_state, ld_metrics = jax.lax.scan(
        sample_with_sigma, init_params, jnp.arange(len(sigmas))
    )
...
def consistent_langevin_dynamics(
    ...
    def langevin_step(params, i):
        ...
        sigma = sigmas[i]
        next_sigma = jnp.where(i < len(sigmas) - 1, sigmas[i + 1], 0.0)

        alpha = epsilon * (sigma / sigmas[-1]) ** 2
        grad = model(state, sigma)
        noise = beta * next_sigma * jax.random.normal(key=step_rng, shape=state.shape)
    ...
    ld_state, ld_metrics = jax.lax.scan(
        langevin_step, init_params, jnp.arange(len(sigmas))
    )
    state, rng = ld_state
...
def diffusion_dynamics(
    ...
    collection_idx = jnp.linspace(1, len(betas), collection_steps).astype(jnp.int32)
    ...
    def sample_with_beta(params, t):
        ...
        infill_noise = jax.random.normal(
            key=infill_noise_rng, shape=infill_samples.shape
        )
        noisy_y = (
            jnp.sqrt(alpha_prod) * infill_samples
            + jnp.sqrt(1 - alpha_prod) * infill_noise
        )
        ...
        state_recon = (
            sqrt_reciprocal_alpha_prod * state - sqrt_alpha_prod_m1 * eps_recon
        )
        state_recon = jnp.clip(state_recon, -1.0, 1.0)
