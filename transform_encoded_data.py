...
flags.DEFINE_string(
    "encoded_data", "~/data/encoded_lmd", "Path to encoded data TFRecord directory."
)
...
flags.DEFINE_enum("output_format", "tfrecord", ["tfrecord", "pkl"], "Shard file type.")

flags.DEFINE_enum(
    "mode", "flatten", ["flatten", "sequences", "decoded"], "Transformation mode."
)
flags.DEFINE_boolean("remove_zeros", True, "Remove zero vectors.")
flags.DEFINE_integer(
    "context_length", 4, "The length of the context window in a sequence."
)
flags.DEFINE_integer("stride", 1, "The stride used for generating sequences.")
flags.DEFINE_integer("max_songs", None, "The maximum number of songs to process.")
flags.DEFINE_integer("max_examples", None, "The maximum number of examples to process.")
...
def toy_sequence_distribution_fn(trajectory_length=10, batch_size=512):
    ...
    deltas = np.expand_dims(step_size, 1).repeat(trajectory_length, axis=1) * np.arange(
        trajectory_length
    ).reshape(trajectory_length, 1)
...
def main(argv):
    ...
    # embed()
    eval_dataset = tf.data.TFRecordDataset(eval_files).map(
        lambda x: tf.py_function(
            lambda binary: pickle.loads(binary.numpy()), [x], tensor_shape
        ),
        num_parallel_calls=tf.data.experimental.AUTOTUNE,
            ...
            if FLAGS.mode == "flatten":
                for vec in song:
                    if FLAGS.remove_zeros and np.linalg.norm(vec) < 1e-6:
                        continue
                    if (
                        FLAGS.max_examples is not None
                        and example_count >= FLAGS.max_examples
                    ):
                ...
                for i in range(0, len(song) - ctx_window, stride):
                    ...
                    if (
                        FLAGS.remove_zeros
                        and np.where(np.linalg.norm(context, axis=1) < 1e-6)[0].any()
                    ):
                        # discard if any latent is 0 vector
                        continue
                    if (
                        FLAGS.max_examples is not None
                        and example_count >= FLAGS.max_examples
                    ):
                        # if we already got too many examples
                        should_terminate = True
                        break

                    example_count += 1  # we got one!
                    contexts.append(context)
                    targets.append(song[i + ctx_window])

            if len(targets) >= FLAGS.shard_size:
                contexts, targets = save_shard(
                    contexts, targets, output_fp.format(FLAGS.output_path, split, count)
                )
                count += 1
