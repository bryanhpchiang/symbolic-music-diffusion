...
def get_dataset(
    ...
    train_ds = train_ds.map(
        partial(deconstruct_dict, problem=problem), num_parallel_calls=AUTOTUNE
    )
    eval_ds = eval_ds.map(
        partial(deconstruct_dict, problem=problem), num_parallel_calls=AUTOTUNE
    )
    ...
    train_min, train_max = 0.0, 1.0
    eval_min, eval_max = 0.0, 1.0
    if normalize:
        ...
        train_ds = train_ds.map(
            lambda example: normalize_dataset(example, train_min, train_max),
            num_parallel_calls=AUTOTUNE,
        )
        eval_ds = eval_ds.map(
            lambda example: normalize_dataset(example, eval_min, eval_max),
            num_parallel_calls=AUTOTUNE,
        )
