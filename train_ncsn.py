...
flags.DEFINE_boolean(
    "early_stopping", False, "Use early stopping to prevent overfitting."
)
flags.DEFINE_float("grad_clip", 1.0, "Max gradient norm for training.")
flags.DEFINE_float("lr_gamma", 0.98, "Gamma for learning rate scheduler.")
flags.DEFINE_integer(
    "lr_schedule_interval", 10000, "Number of steps between LR changes."
)

# Model
flags.DEFINE_string(
    "architecture", "TransformerDDPM", "Class name of model architecture."
)
...
# Noise schedule
flags.DEFINE_float(
    "sigma_begin", 1.0, "Starting variance for noise schedule."
)  # Technique 1
...
flags.DEFINE_integer(
    "num_sigmas", 15, "Number of sigma values (L) in noise schedule."
)  # Technique 2

# Langevin dynamics (NCSN only)
flags.DEFINE_integer("ld_steps", 100, "Number of steps for annealed Langevin dynamics.")
flags.DEFINE_float(
    "ld_epsilon", 2e-6, "Step size for annealed Langevin dynamics."
)  # Technique 4

# Sampling
flags.DEFINE_enum(
    "sampling", "ald", ["ald", "cas", "ddpm"], "Sampling algorithm to use."
)
flags.DEFINE_boolean(
...
flags.DEFINE_enum("problem", "toy", ["toy", "mnist", "vae"], "Problem to solve.")
flags.DEFINE_string(
    "dataset",
    "./output/mix2d",
...
flags.DEFINE_integer("snapshot_freq", 5000, "Evaluation and checkpoint frequency.")
flags.DEFINE_boolean(
    "snapshot_sampling", True, "Sample from score network during evaluation."
)
flags.DEFINE_integer("eval_samples", 3000, "Number of samples to generate.")
flags.DEFINE_integer("checkpoints_to_keep", 50, "Number of checkpoints to keep.")
flags.DEFINE_boolean(
    "save_ckpt", True, "Save model checkpoints at each evaluation step."
)
flags.DEFINE_string("model_dir", "./save/ncsn", "Directory to store model data.")
...
def create_model(rng, input_shape, model_kwargs, batch_size=32, verbose=False):
    ...
    output, initial_params = module.init_by_shape(
        rng,
        [
            ((batch_size, *input_shape), jnp.float32),
...
def train(train_batches, valid_batches, sigmas, output_dir=None, verbose=True):
    ...
    model = create_model(
        model_rng, input_shape, model_kwargs, batch_size, verbose=verbose
    )
    ...
    for epoch in range(FLAGS.epochs):
        ...
        for step, batch in enumerate(tfds.as_numpy(train_batches)):
            ...
            if (
                step % FLAGS.snapshot_freq == 0 and step > 0
            ) or step == train_batches.examples - 1:

                sampling_step += 1

                rng, eval_rng = jax.random.split(rng)
                eval_metrics = evaluate(
                    valid_batches, optimizer.target, sigmas, eval_rng
                )
                train_utils.log_metrics(
                    eval_metrics,
                    global_step,
                ...
                if FLAGS.early_stopping and early_stop.should_stop:
                    logging.info(
                        "EARLY STOP: Ended training after %s epochs.", epoch + 1
                    )
                    return
                ...
                if FLAGS.snapshot_sampling:
                    ...
                    init = collection[0]
                    real = (
                        valid_batches.unbatch()
                        .shuffle(8 * batch_size)
                        .take(FLAGS.eval_samples)
                    )
                    real = np.stack([ex for ex in tfds.as_numpy(real)])
                    real = input_pipeline.inverse_data_transform(
                        real,
                        FLAGS.normalize,
                        pca,
                        valid_batches.min,
                    ...
                    init = input_pipeline.inverse_data_transform(
                        init,
                        FLAGS.normalize,
                        pca,
                        train_batches.min,
                    ...
                    if FLAGS.problem == "toy":
                        ...
                        if len(input_shape) == 1 and FLAGS.sampling != "ddpm":
                            for sigma in sigmas:
                                score_buf = plot_utils.score_field_2d(
                                    optimizer.target, sigma=sigma, scale=8
                                )
                                score_im = tf.image.decode_png(
                                    score_buf.getvalue(), channels=4
                                )
                                eval_writer.image(
                                    "score_sigma={:.4f}".format(sigma),
                                    score_im,
                                    step=global_step,
                                )
            ...
            if FLAGS.max_steps is not None and global_step >= FLAGS.max_steps:
                return optimizer
...
def sample(
    ...
    if sampling == "ddpm":  # TODO: change this based on infilling
        ...
        init = jax.random.uniform(
            key=init_rng, shape=(num_samples, *sample_shape), minval=-rho, maxval=rho
        )
...
def main(argv):
    ...
    # Noise schedule
    noise_schedule = ebm_utils.create_noise_schedule(
        FLAGS.sigma_begin,
        FLAGS.sigma_end,
        FLAGS.num_sigmas,
