...
flags.DEFINE_integer(
    "model_step", None, "Training step to restore model checkpoint from."
)

# Metrics.
flags.DEFINE_boolean(
    "compute_metrics", False, "Compute evaluation metrics for generated samples."
)
flags.DEFINE_boolean(
    "compute_final_only", False, "Do not include metrics for intermediate samples."
)
...
def evaluate(writer, real, collection, baseline, valid_real):
    ...
    if (
        collection[-1].min() < -1.0
        or collection[-1].max() > 1.0
        or real.min() < -1
        or real.max() > 1.0
    ):
        warnings.warn("Normalize test samples and generated samples to [-1, 1] range.")

    # embed()
    # In [1]: collection.shape
    # Out[1]: (41, 50, 32, 42)

    # In [2]: real.shape
    # Out[2]: (50, 32, 42)

    gen_test_points = collection[
        np.linspace(0, len(collection) - 1, 20).astype(np.uint32)
    ]
    ...
    # self-similarity
    # frechet distance (latent space)

    for model, test_points in [
        # ("baseline", [baseline]),
        ...
        for i, samples in enumerate(test_points):
            ...
            # buf = io.BytesIO()
            # metrics.prd.plot(
            #     [prd_dist, prd_init, prd_perfect], [model, "noise", "real"]
            # )
            ...
            writer.scalar(
                f"{log_dir}frechet_distance/{model}_mean",
                jnp.mean(frechet_dist),
                step=0,
            )
            writer.scalar(
                f"{log_dir}frechet_distance/{model}_std", jnp.std(frechet_dist), step=0
            )

            mmd_rbf = jnp.array([metrics.mmd_rbf(r, s) for r, s in zip(real, samples)])
            writer.scalar(f"{log_dir}mmd_rbf/{model}_mean", jnp.mean(mmd_rbf), step=0)
            writer.scalar(f"{log_dir}mmd_rbf/{model}_std", jnp.std(mmd_rbf), step=0)

            mmd_polynomial = jnp.array(
                [metrics.mmd_polynomial(r, s) for r, s in zip(real, samples)]
            )
            writer.scalar(
                f"{log_dir}mmd_polynomial/{model}_mean",
                jnp.mean(mmd_polynomial),
                step=0,
            )
            writer.scalar(
                f"{log_dir}mmd_polynomial/{model}_std", jnp.std(mmd_polynomial), step=0
            )
            # embed()
    ...
    stats = {
        ...
        # "realism": realism,
        "frechet_dist": jnp.mean(frechet_dist),
        "mmd_rbf": jnp.mean(mmd_rbf),
        "mmd_polynomial": jnp.mean(mmd_polynomial),
    }
    return stats
...
def diffusion_stochastic_encoder(samples, rng_seed=1):
    ...
    rng = jax.random.PRNGKey(rng_seed)
    betas = ebm_utils.create_noise_schedule(
        FLAGS.sigma_begin,
        FLAGS.sigma_end,
        FLAGS.num_sigmas,
        schedule=FLAGS.schedule_type,
...
def diffusion_decoder(z_list, rng_seed=1):
    ...
    model = train_ncsn.create_model(
        model_rng, z_list[0].shape[1:], model_kwargs, batch_size=1, verbose=True
    )
    optimizer = train_ncsn.create_optimizer(model, 0)
    ema = train_utils.EMAHelper(mu=0, params=model.params)
    early_stop = train_utils.EarlyStopping()
...
def generate_samples(sample_shape, num_samples, rng_seed=1, checkpoint_step=None):
    ...
    model = train_ncsn.create_model(
        model_rng, sample_shape, model_kwargs, batch_size=1, verbose=True
    )
    optimizer = train_ncsn.create_optimizer(model, 0)
    ema = train_utils.EMAHelper(mu=0, params=model.params)
    early_stop = train_utils.EarlyStopping()
...
def main(argv):
    ...
    if FLAGS.infill:  # Infilling.
        ...
        interp_zs = [
            (1 - alpha) * starts_z + alpha * goals_z
    ...
    if FLAGS.flush:
        ...
        if not FLAGS.interpolate:
            ...
            data_utils.save(collection_t, os.path.join(log_dir, "ncsn/collection.pkl"))
