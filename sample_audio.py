...
@ray.remote
def parallel_synth(song, i, ns_dir, audio_dir, image_dir, include_wav, include_plots):
...
def main(argv):
    ...
    if FLAGS.melody:
        ...
        ckpt = os.path.expanduser("cat-mel_2bar_big.tar")
        vae_model = TrainedModel(
            model_config, batch_size=1, checkpoint_dir_or_path=ckpt
        )
    else:
        model_config = config.MUSIC_VAE_CONFIG["multi-0min-1-big"]
        ckpt = os.path.expanduser("~/checkpoints/multitrack/fb512_0trackmin/model.ckpt")
        vae_model = TrainedModel(
            model_config, batch_size=1, checkpoint_dir_or_path=ckpt
        )
    ...
    interp_baseline = [
        ...
        for alpha in np.linspace(0.0, 1.0, 16 + 2)
    ]
    ...
    eval_seqs = {}
    for sample_split, sample_emb in (
        ("real", real),
        ("gen", generated),
        ("prior", prior),
        ("interp", interp_baseline),
    ):
        ...
        sequences = decode_emb(
            sample_emb[: FLAGS.n_synth],
            vae_model,
